...
public class HMAC_MD5 {
	private static final Log logger = LogFactory.getLog(HMAC_MD5.class);
	/**
	...
	public static void main(String arg[])
	{
		...
		Debug.info(logger,"Test Vectors from RFC 2104 - HMAC: Keyed-Hashing for Message Authentication");
		Debug.info(logger,"This test uses HMAC-MD5.");

		Debug.info(logger,"Test #1:");
		// Test #1:
		byte key1[] = {
		...
		try {
			...
			Debug.info(logger,"Calculated hash 0X" + hm);
			Debug.info(logger,"  Expected hash " + expectedHash);
	
			// Test #2
			Debug.info(logger,"Test #2:");
			...
			Debug.info(logger,"Calculated hash 0X" + hm);
			Debug.info(logger,"  Expected hash " + expectedHash);
			Debug.info(logger,"Signature Verification: " + hm.verify(eh));
		} catch (NoSuchAlgorithmException nsae) {
			Debug.exception(logger,nsae);
		}
	}
