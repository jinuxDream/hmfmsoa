 ...
 * <p>创建时间: 2010-12-13 上午11:56:51</p>
 * @author 产品开发部
 * @version 2.0
 * MD5
 */
public class MD5 extends AbstractUtil {
	private static final Log logger = LogFactory.getLog(MD5.class);
	public String getMD5ofStr(String s, String s1) {
		...
		for(i = 0; i < abyte2.length; i++) {
			abyte0[i] = (byte)(abyte2[i] ^ 0x36);
			abyte1[i] = (byte)(abyte2[i] ^ 0x5c);
		}
		...
		md5Init();
		md5Update(abyte0, 64);
		md5Update(s1.getBytes(), s1.length());
		md5Final();
		md5Init();
		md5Update(abyte1, 64);
		md5Update(digest, digest.length);
		md5Final();
		digestHexStr = "";
		for(int j = 0; j < 16; j++)
			digestHexStr += byteHEX(digest[j]);
	...
	public static void main(String args[]) {
		...
		MD5 md5 = new MD5();
		Debug.info(logger,"MD5(" + args[0] + ")=" + md5.getMD5ofStr(args[0], args[1]));
	}
