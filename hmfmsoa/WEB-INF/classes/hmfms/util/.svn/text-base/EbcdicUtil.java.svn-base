...
/**
 * 
 * ebcdic码为as400/os390主机常用编码，在微机处理时需要进行对该码的转换才能正确显示中英文等字符。
 * ebcdic码对中文的处理，会在连续中文的前面加上0E在尾部加上0F来区别.
 * jdk在1.4版本以后都有对他的转码方法，对应java版本他的名字叫1388码，所以可以通过new String(bytes,"1388")进行转换.
 * 
 */
/**
 * <p>标    题: 核心框架</p>
 * <p>描    述: ebcdic编码实用类，ebcdic码为as400/os390主机常用编码，在微机处理时需要进行对该码的转换才能正确显示中英文等字符。
 * ebcdic码对中文的处理，会在连续中文的前面加上0E在尾部加上0F来区别.
 * jdk在1.4版本以后都有对他的转码方法，对应java版本他的名字叫1388码，所以可以通过new String(bytes,"1388")进行转换.</p>
 * <p>版    权: Copyright (c) 2010</p>
 * <p>公    司: 上海泓智信息科技有限公司</p>
 * <p>创建时间: 2010-12-13 下午12:29:50</p>
 * @author 产品开发部
 * @version 2.0
 * EbcdicUtil
 */
public class EbcdicUtil extends AbstractUtil {
	private static final Log logger = LogFactory.getLog(EbcdicUtil.class);
	private EbcdicUtil() {
	...
	/**
	 * 使用JAVA内置功能完成字符串的EBCDIC->ASCII转换转码，支持中文。
	 * @param ebcdic
	 * @return
	 */
	public static String EBCDICToASCIICp(String ebcdic) {

		return EBCDICToASCIICp(ebcdic.getBytes());
	}

	/**
	 * 使用JAVA内置功能完成字节流的EBCDIC->ASCII转换转码，支持中文。
	 * @param ebcdic
	 * @return
	 */
	public static String EBCDICToASCIICp(byte[] ebcdic) {

		try {
			String tmp = new String(ebcdic, "Cp935");//1388
			//Debug.info(logger,"Ebcdic to ascii："+tmp);
			return tmp;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		String fromfile = "D:\\project\\hmfms\\City\\青岛\\Ebcdic转码\\EBCDIC\\EBCDIC";
		FileInputStream in = new FileInputStream(new File(fromfile));
		int tempint, i = 0;
		byte[] tempbytes = new byte[in.available()];
		while( (tempint = in.read()) != -1 )
		...
		Debug.info(logger,"原始Ebcdic内容：    " + new String(tempbytes));

		String ea = EBCDICToASCIICp(tempbytes);

		Debug.info(logger,"Ebcdic to ascii：" + ea);
	}
}