...
public class MoneyUtil extends AbstractUtil {
	...
	/**
	 * 转化为大写汉字的金额值
	 * @param digit 最多有两位小数,本方法最大支持金额999999999999.99
	...
	private static void dividedString(String amt) {
		...
		if( amt.indexOf(".") == -1 ) {
			...
			/*
			 Debug.info(logger,"");
			 Debug.info(logger,"无小数点");
			 Debug.info(logger,"小数点位置："+indexOfPoint);
			 Debug.info(logger,"整数长度："+lengthOfInt);
			 */
		}
		else {
			...
			/*
			 Debug.info(logger,"");
			 Debug.info(logger,"有小数点");
			 Debug.info(logger,"小数点位置："+indexOfPoint);
			 Debug.info(logger,"整数长度："+lengthOfInt);
			 Debug.info(logger,"小数部分："+decimalPart);
			 */
		}
	}
	...
	private static String convertIntPart(String intPart) {
		...
		if( lengthOfInt % 3 == 0 ) {
			...
			/*
			 Debug.info(logger,"");
			 Debug.info(logger,"除三余0");
			 Debug.info(logger,"逗号数量："+numOfCommer);
			 Debug.info(logger,"首个逗号的位置："+indexOfFirstCommer);
			 */
		}
		else {
			...
			/*
			 Debug.info(logger,"");
			 Debug.info(logger,"除三余"+lengthOfInt%3);
			 Debug.info(logger,"逗号数量："+numOfCommer);
			 Debug.info(logger,"首个逗号的位置："+indexOfFirstCommer);
			 */
		}
	...
	private static String convertDecimalPart(String decimalPart, int decimalNum) {
		...
		if( decimalPart.length() > decimalNum ) {
			//Debug.info(logger,"超过"+decimalNum+"位小数");
		...
		//小于n位小数
			...
			//Debug.info(logger,"小于"+decimalNum+"位小数");
			for(int i = 0; i < (decimalNum - decimalPart.length()); i++)
				decimalPart = decimalPart + "0";
			return "." + decimalPart;
		}
		//else Debug.info(logger,"正好"+decimalNum+"位小数");
		return "." + decimalPart;
	}
