	...
	function SlickGrid(container, data, columns, options) {
		...
		function disableSelection($target) {
			if ($target && $target.jquery) {
				$target.attr("unselectable", "on").css("MozUserSelect", "none").bind("selectstart.ui",
				function() {
					return false;
		...
		function setupColumnResize() {
			...
			columnElements.each(function(i, e) {
				...
				$col = $(e);
				$("<div class='slick-resizable-handle' />").appendTo(e).bind("dragstart",
				function(e, dd) {
					if (!getEditorLock().commitCurrentEdit()) {
						return false;
					}
		...
		function applyColumnHeaderWidths() {
			...
			var h;
			for (var i = 0,
			headers = $headers.children(), ii = headers.length; i < ii; i++) {
				h = $(headers[i]);
		...
		function cleanUpAndRenderCells(range) {
			...
			var colspan;
			for (var row = range.top,
			btm = range.bottom; row <= btm; row++) {
